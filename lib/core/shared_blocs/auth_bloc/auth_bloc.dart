...
class AuthBloc extends HydratedBloc<AuthEvent, AuthState> {
  ...
  Future<void> _onSignIn(SignIn event, emit) async {
    try {
      if (state.tempUserData.email != "" &&
          state.tempUserData.email != null &&
          state.tempUserData.password != "" &&
          state.tempUserData.password != null &&
  ...
  Future<void> _onFormFieldChanged(
    ...
    emit(state.copyWith(
      ...
      firstName: event.fieldType == FieldType.firstName
          ? event.value
          : state.tempUserData.firstName,
      lastName: event.fieldType == FieldType.lastName
          ? event.value
          : state.tempUserData.lastName,
      email: event.fieldType == FieldType.email
          ? event.value
          : state.tempUserData.email,
      password: event.fieldType == FieldType.password
          ? event.value
          : state.tempUserData.password,
    )));
  }
  ...
  Future<void> _onSignUp(
    ...
    if ((state.tempUserData.firstName == null ||
            state.tempUserData.firstName == "") &&
        !RegExp(r'[!@#<>?":_`~;[\]\\|=+ )(*&^%0-9-]')
            .hasMatch(state.tempUserData.firstName ?? "")) {
      AlertBox().showError(ErrorStrings.invalidFirstName);
    } else if (state.tempUserData.lastName == null ||
        state.tempUserData.lastName == "" &&
            !RegExp(r'[!@#<>?":_`~;[\]\\|=+ )(*&^%0-9-]')
                .hasMatch(state.tempUserData.lastName ?? "")) {
      AlertBox().showError(ErrorStrings.invalidLastName);
    } else if (state.tempUserData.email == null ||
        state.tempUserData.email == "" &&
            !RegExp(
              r"^[a-zA-Z0-9.a-zA-Z0-9.!#$%&'*+-/=?^_`{|}~]+@[a-zA-Z0-9]+\.[a-zA-Z]+",
            ).hasMatch(state.tempUserData.email ?? "")) {
      AlertBox().showError(ErrorStrings.invalidEmail);
    } else if (state.tempUserData.password == null ||
        state.tempUserData.password == "") {
      AlertBox().showError(ErrorStrings.pleaseEnterPassword);
    } else {
